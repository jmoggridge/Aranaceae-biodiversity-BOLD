...
# tabulate n specimens and n bins by country for arranging countries by n specimens and bins
...
write_rds(Araneae.df.bins, "./data/Araneae.bins.rds", compress = 'gz')
...
write_rds(Araneae.country.bins, "./data/Araneae.country.bins.rds", compress = 'gz')
...
write_rds(Araneae.zone.bins, "./data/Araneae.zone.bins.rds", compress = 'gz')
```
...
<!-- this chunk takes a very long time -->
...
Araneae.rar <- tibble(y = Araneae.rar[[1]]) %>%
  mutate(x=row_number())
write_rds(Araneae.rar, "./data/Araneae.rarefaction.rds", compress = 'gz')
...
write_rds(Araneae.zone.rar, "./data/Araneae.zones.rarefaction.rds", compress = 'gz')
...
Araneae.country.rar <- tibble(Country = rownames(Araneae.country.bins),
  ...
  group_by(Country) %>%
  mutate(sample_size = row_number())
write_rds(Araneae.country.rar, "./data/Araneae.country.rarefaction.rds", compress = 'gz')
```
...
Specaccum.country <- tibble(
  ...
  mutate(sd1 = ifelse(richness-sd>=0, richness-sd, 0),
         sd2 = richness+sd)
write_rds(Specaccum.country, "./data/Specaccum.country.rds", compress = 'gz')
...
Specaccum.zone <- tibble(
  ...
  mutate(sd1 = ifelse(richness-sd>=0, richness-sd, 0),
         sd2 = richness+sd)
write_rds(Specaccum.zone, "./data/Specaccum.zone.rds", compress = 'gz')
...
**Figure 5.** NMDS of spider BIN abundances in four latitude zones using Bray distance and Wisconsin standardization of square-root-transformed abundances (red crosses repesent individual spider BIN groups).
...
**Figure 6.** Network of latitude-zoned spider assemblages linked by shared barcodes shows that similarity increases with latitude. Vertices represent latitude zone and BIN richness; edges are weighted by the number of shared BINs between zones.
